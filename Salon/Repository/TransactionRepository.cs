...
namespace Salon.Repository
{
    public class TransactionRepository
    {
        ...
        public async Task<IEnumerable<TransactionModel>> ShowAllTransactionAsync(DateTime start, DateTime end)
        {
            using (var con = Database.GetConnection())
            {
                var sql = @"SELECT 
                        ...
                        WHERE t.timestamp BETWEEN @start AND @end
                        GROUP BY t.transaction_id";
                var result = await con.QueryAsync<TransactionModel>(sql, new { start, end });
        ...
        public async Task<IEnumerable<TransactionModel>> GetTransactionAsync() 
        {
            using (var con = Database.GetConnection())
            {
                var sql = "SELECT * FROM tbl_transaction";
        ...
        public async Task<IEnumerable<TransactionModel>> GetTransactionAsync(DateTime start_date, DateTime end_date)
        {
            using (var con = Database.GetConnection())
            {
                var sql = "SELECT * FROM tbl_transaction WHERE timestamp BETWEEN @start_date AND @end_date";

                var result = await con.QueryAsync<TransactionModel>(sql, new { start_date = start_date, end_date = end_date });
